...
\defbibnote{sources-cds}{Cette liste contient les cotes de l'inventaire numérique \textit{La correspondance de Constance de Salm (1767-1845). Inventaire du fonds Salm de la Société des Amis du Vieux Toulon et de sa Région et du fonds Constance de Salm, Archiv Schloss Dyck (Mitgliedsarchiv der Vereinigten Adelsarchive im Rheinland e.V.). Édition numérique}, DHI Paris, 2021, \textsf{https://constance-de-salm.de}~:}
	...
	\chapter[HTR~: mise en œuvre]{Mettre en œuvre l'HTR \\ \large pour la correspondance de \gls{cds}}
		...
		\section[La segmentation]{La segmentation~: reconnaissance des zones de texte et des lignes d'écriture}
			...
			\subsection{Définir une structure de document idéale en vue de l'édition}
				...
				Par ailleurs, les réflexions suscitées par ces catégories lors du point d'étape du 22 avril 2022\footnote{Ont participé à cette discussion Anne Baillot, Floriane Chiffoleau, Pauline Spychala, Evan Virevialle et moi-même.} ont fait émerger l'idée d'une simplification de cette ontologie par régions autour de trois notions principales marquant l'ouverture, le corps et la fin d'une lettre~:
				...
				Il fallait en outre envisager l'annotation des éléments périphériques (annotations et systèmes de numérotation divers)~:
				\textit{\selectlanguage{english}
				\begin{enumerate}
					\item Annotations;
					\item Numbering.
    		...
    		\subsection{Résultats des entraînements}
    			...
    			Les recherches actuelles portant sur ce domaine d'ingénierie permettent d'espérer de meilleurs résultats. Dans un article sous presse, T.~Clérice propose une nouvelle méthode d'entraînement fondée sur la technologie de détection d'objet plutôt que sur celle de la classification des pixels, ainsi qu'un module dénommé YALTAi (\textit{You Actually Look Twice At it}), grâce auquel des modèles de segmentation entraînés par détection d'objet avec l'application YOLOv5, dont les performances ont été évaluées bien meilleures que celles de Kraken, sont désormais rendus compatibles avec l'application Kraken et pourraient ainsi être intégrés à la chaîne de traitement\footcite{clericeYouActuallyLook2022}.
...
<seriesStmt>
  ...
  <respStmt>
    ...
    \nocite{biayEditerCorrespondanceConstance2022a}
