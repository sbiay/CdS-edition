	...
	\chapter[HTR~: mise en œuvre]{Mettre en œuvre l'\gls{htr} \\ \large pour la correspondance de \gls{cds}}
		...
		\section[La segmentation]{La segmentation~: reconnaissance des zones de texte et des lignes d'écriture}
    		...
    		\subsection{Résultats des entraînements}
	    		...
	    		La figure \ref{seg-copie-deux-04-229} montre l'application du modèle entraîné\footnote{Le modèle est déposé à l'adresse suivante~: \url{https://github.com/sbiay/CdS-edition/blob/main/htr/modeles-seg/copie-deux-04.mlmodel}.} à une nouvelle double page et permet de constater les résultats de cette personnalisation du modèle par défaut de Kraken.
    			...
    			Le nouveau modèle est parvenu à définir de manière correcte les manchettes (entourées en bleu) et le corps des lettres, réunissant toutes les parties, du titre à la signature, en leur attribuant la bonne typologie de région. En revanche le petit numéro situé en bas de la page de droite, bien que repéré par le segmenteur, n'a pas reçu la bonne annotation, puisqu'il a été catégorisé de la même manière que la manchette. Concernant les lignes d'écriture, on remarque qu'elles ont été globalement bien identifiées. Chose importante, toutes les lignes que l'on avait annotées comme \textsf{DefaultLine} ont été correctement annotées automatiquement dans cette évaluation.
    			...
    			Bien que très imparfait, ce résultat n'en constitue pas moins un relatif succès de l'entraînement, car ce résultat est tout à fait exploitable. Il représente une réelle économie de temps pour la correction manuelle et l'annotation des zones, dans la mesure où il parvient malgré tout à annoter correctement la manchette, le corps de la lettre et les lignes par défaut.
    			
    			On a tenté de confirmé ces résultats encourageants en apportant de nouvelles pages au jeu d'entraînement. Mais contrairement à ce que l'on espérait de ce nouvel entraînement où le jeu initial avait été augmenté de 61 doubles pages corrigées à la main
