...
\begin{document}
	...
	\chapter[HTR]{Reconnaissance automatique des écritures manuscrites \\ \large (\gls{htr})}
		...
		\end{enumerate}
		% On peut ajouter les deux autres étapes indiquées dans \cite{stokesEScriptoriumVREManuscript2021} :
		...
		% Repose sur le machine learning et donc sur la constitution de vérités de terrain ou ground truth.
		% Il en découle que la pertinence de cette méthode doit être évaluée à la lumière de sa rentabilité : dans quelle mesure cela ne serait pas mieux de faire tout à la main.
		...
		\section{La reconnaissance des caractères}
			...
			\subsection{Transcription manuelle \textit{versus} trasncription automatique~: quelle bonne méthode pour l'entraînement~?}
				\textbf{Cette section est à écrire.}
				% On a testé la méthode récursive pour l'entraînement des modèles : transcrire, entraîner, prédire, corriger, entraîner (cf.~\cite{stokesEScriptoriumVREManuscript2021})
				% En réalité, on perdait du temps !
				% En tout cas il faut utiliser eScriptorium comme interface de segmentation et de transcription, même si tout le processus est manuel.
